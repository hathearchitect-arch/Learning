...
import type { Suggestion, DBMessage, Chat, User, Organization } from './types';
...
export async function getAgent(agentId: string) {
  try {
    ...
    if (agentData.length === 0) {
      throw new ChatSDKError('bad_request:database', 'Invalid agentId');
    }
    return agentData[0];
  } catch (error) {
    throw new ChatSDKError(
      'bad_request:database',
      'Failed to get agent by agentId',
    );
  }
}
...
export async function checkAgentAccess(userId: string, agentId: string) {
  try {
    const agentAccess = await db
      .select()
      .from(agentUser)
      .where(and(eq(agentUser.userId, userId), eq(agentUser.agentId, agentId)));
    return agentAccess;
  } catch (error) {
    throw new ChatSDKError(
      'bad_request:database',
      'Failed to find agent access by userId and agentId',
    );
  }
}
...
export async function getOrganizationsByUserId(
  ...
  try {
    const userOrgList = db
      .select({ data: member.organizationId })
      .from(member)
    ...
    const orgList = await db
      .select()
      .from(organization)
      .where(inArray(organization.id, userOrgList));
    return orgList;
  } catch (error) {
    throw new ChatSDKError(
      'bad_request:database',
...
export async function checkOrganizationMembership(
  ...
  try {
    ...
    return membershipCheck.length > 0;
  } catch (error) {
    console.log('error checking organization membership: ', error);
    throw new ChatSDKError(
      'not_found:auth',
      'Unable to check that user is a member of organization',
    );
  }
}
...
export async function getOrganizationById(
  ...
  try {
    ...
    console.error('failed to get organization id, ', organizationId, error);
    throw new ChatSDKError(
      'bad_request:database',
  ...
  try {
    ...
    const query = (whereCondition?: SQL<any>) =>
      db
        ...
        .where(
          whereCondition
            ? and(whereCondition, eq(chat.agentId, agentId))
            : eq(chat.agentId, agentId),
        )
        .orderBy(desc(chat.createdAt))
        .limit(extendedLimit);
    ...
    if (organizationId) {
      const [selectedChat] = await db
        .select({
          ...
          title: chat.title,
          userId: chat.userId,
          impersonatedById: chat.impersonatedById,
          agentId: chat.agentId,
          visibility: chat.visibility,
        })
        .from(chat)
        .innerJoin(agent, eq(chat.agentId, agent.id))
        .where(and(eq(chat.id, id), eq(agent.organizationId, organizationId)));
      return selectedChat;
    } else {
      const [selectedChat] = await db
        .select()
